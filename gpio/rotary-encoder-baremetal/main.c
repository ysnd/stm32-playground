...
void gpio_init() {
    ...
    AFIO->EXTICR[0] &= ~(0xF << 8);//map exti1 ke pa1 

    //trigg 
    EXTI->IMR |= (1 << 0) | (1 << 2); //unmask
    EXTI->FTSR |= (1 << 0) | (1 << 2); //falling trigger
...
void EXTI0_IRQHandler(void) {
    if (EXTI->PR & (1 << 0)) {
        if ((ms_ticks - last_enc) > 2) {
            if (GPIOA->IDR & (1 << 1)) { // read dt
                count++;
            }
            else {
                count--;
